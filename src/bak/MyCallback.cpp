...
void CB_MergeRet(const std::string& pathSave, json& jsRet) {
    ...
    stmSaveRet << "-->" << jsRet.dump() << std::endl << std::endl;
    stmSaveRet.close();
...
void CB_JSON(ASD_EVENT_TYPE eventType, void* msg, int32_t msgLen) {
    ...
    if (eventType == ASD_EVENT_DMS) {
        ...
        CB_MsgPro_ASD_OMS_RESULT(UpLocalSync()(g_Sync_Putdata2Callback_OOMS, mtx_oomsput_ooms_callbackinfo),
    ...
    else if(eventType ==ASD_EVENT_DMS_TIMER)
    {
        ...
        CB_MsgPro_ASD_DMS_AGE_GENDER(UpLocalSync()(g_Sync_Putdata2Callback_DMS, mtx_dmsput_dms_callbackinfo),
