...
#include <stdint.h>
#include <sys/types.h>
#include "pmem_id.h" //<amss/pmem_id.h>
#define IN 
#define OUT
...
FUNCTION pmem_get_phandle

@brief
This function gets the pmem handle for a given virtual address and the 
corresponding PMEM_ID.

@param [in] ptr     	address of memory (VA) previously allocated by pmem_malloc 
                    	or pmem_malloc_ext.

@param [in] id      	pmem ID of the component, this should be the same as the ID 
                    	with which the buffer was allocated with.

@param [out] phandle    pmem handle for the given VA and pmem ID.

@dependencies
None

@return 0 on success, -1 or EINVAL otherwise

@sideeffects
None

==========================================================================*/
int pmem_get_phandle
(
 void* IN vaddr,            /* Virtual Address */
 uint32_t IN id,            /* PMEM ID of the MM component */
 pmem_handle_t* OUT phandle /* PMEM handle of the MM component */
);
